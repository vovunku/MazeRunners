...
class GameVisitor:
    ...
    def visit_move_c(self, command):
        self.display.message(
            "{0} successfully moved {1}".format(self.player.id, command.move_strategy.direction_type.lower()))
    ...
    def visit_wall_stop_c(self, command):
        self.display.message(
            "{0} didn't move {1} because of wall".format(self.player.id, command.move_strategy.direction_type.lower()))
        command.execute(self.player)
